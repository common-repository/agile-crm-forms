...
add_action('wp_enqueue_styles', 'agilecrm_forms_css' );
/**
 * Register style sheet.
 */
function agilecrm_forms_css() {
   wp_enqueue_style( 'agilecrm-forms', plugins_url( '/css/style.css', __FILE__ ) );
}
add_action( 'wp', 'agilecrm_forms_landing_page_setpup', 10, 0 ); 
function agilecrm_forms_landing_page_setpup(){
    if(!is_admin()){
        global $post;
        $forms_landing_page = get_post_meta($post->ID, 'forms_landing_page',true);
        if($forms_landing_page != ""){
            $domain = (sanitize_text_field(get_option( "forms_agile_domain" )));
            $email= (sanitize_email(get_option( "forms_agile_email" )));
            $rest_api = (sanitize_text_field(get_option( "forms_agile_rest_api" )));
            if($domain != "" && $email != "" && $rest_api != ""){
               $request  = wp_remote_get('https://'.$domain.'.agilecrm.com/landing/'.$forms_landing_page);
...
add_action( 'init', 'agilecrm_forms_list_agile_form', 10, 0 ); 
function agilecrm_forms_list_agile_form(){
...
add_action('admin_menu', 'agilecrm_forms_create_menu');
function agilecrm_forms_create_menu() {
     add_menu_page('Agile CRM Forms', 'Agile CRM Forms', 'administrator', 'agilecrm_forms_dashboard', 'agilecrm_forms_dashboard_page',plugins_url( 'agile-crm-forms/images/icon.png' ),0);
    add_submenu_page('agilecrm_forms_dashboard', 'Home', 'Home', 'administrator', 'agilecrm_forms_dashboard', 'agilecrm_forms_dashboard_page');
    add_submenu_page('agilecrm_forms_dashboard','Web Rules','Web Rules','administrator', 'agilecrm_forms_webrules', 'agilecrm_forms_webrules_page' );
    add_submenu_page('agilecrm_forms_dashboard','Form Builder','Form Builder','administrator', 'agilecrm_forms_formbuilder', 'agilecrm_forms_formbuilder_page' );
    add_submenu_page('agilecrm_forms_dashboard','Landing Pages','Landing Pages','administrator', 'agilecrm_forms_landing', 'agilecrm_forms_landing_page' );
    add_submenu_page('agilecrm_forms_dashboard','Email Campaigns','Email Campaigns','administrator', 'agilecrm_forms_email', 'agilecrm_forms_email_page' );
    add_submenu_page('agilecrm_forms_dashboard','Web Stats','Web Stats','administrator', 'agilecrm_forms_webstats', 'agilecrm_forms_webstats_page' );
    add_submenu_page('agilecrm_forms_dashboard','Refer a Friend','Refer a Friend','administrator', 'agilecrm_forms_refer', 'agilecrm_forms_refer_page' );
    add_submenu_page('agilecrm_forms_dashboard','Settings','Settings','administrator', 'agilecrm_forms_settings', 'agilecrm_forms_settings_page' );
}
function agilecrm_forms_header($domain){
 $forms_agile_email = (sanitize_email(get_option( "forms_agile_email" )));
 $forms_agile_password = (sanitize_text_field(get_option( "forms_agile_password" )));
 agilecrm_forms_css();
 $page_name = $_GET['page'];
 if($page_name == 'agilecrm_forms_dashboard')
 {
  agilecrm_forms_hidedata_js(); ?> <div id="agilewrapper" class="textaligncenter">
<?php echo "<img src='".plugins_url( '/images/agile-crm.png', __FILE__ )."'  title='Agile Crm logo'/>"; ?> </div>
<?php $webstats= get_option( "forms_agile_webstats" );
    if($webstats === false){
        add_option( "forms_agile_webstats", 1, $deprecated, $autoload );
        $webstats= 1;
    }
    $webrules = get_option( "forms_agile_webrules" );
    if($webrules === false){
        add_option( "forms_agile_webrules", 1, $deprecated, $autoload );
       $webrules = 1;
    }
    $status = $_GET['status'];
    if($status){
...
function agilecrm_forms_dashboard_page(){
    $rest_api = (sanitize_text_field(get_option( "forms_agile_rest_api" )));
    $js_api = (sanitize_text_field(get_option( "forms_agile_js_api" )));
    $domain = (sanitize_text_field(get_option( "forms_agile_domain" )));
    $email= (sanitize_email(get_option( "forms_agile_email" )));
    $password= (sanitize_text_field(get_option( "forms_agile_password" )));
   if(empty($rest_api) || empty($js_api) || empty($domain) || empty($email)){
        agilecrm_forms_settings_page();
    } else {
    $deprecated = null;
    $autoload = 'no';
    if($_POST["featuresform"] && check_admin_referer( 'agilecrm_forms_nonce_action', 'agilecrm_forms_nonce_field' )){
        $webrules = intval($_POST["webrules"]);
        $webstats = intval($_POST["webstats"]);
        if ( get_option( "forms_agile_webrules" ) !== false ) {
            update_option( "forms_agile_webrules", $webrules);
        } else {
            add_option( "forms_agile_webrules", $webrules, $deprecated, $autoload );
        }
             $pages = get_pages(); 
             foreach ( $pages as $page ) {
                 if($webrules == "0")
                     update_post_meta($page->ID , 'forms_enable_webrule', 2);
                 else
                     update_post_meta($page->ID , 'forms_enable_webrule', 1);
             }
        if ( get_option( "forms_agile_webstats" ) !== false ) {
            update_option( "forms_agile_webstats", $webstats);
        } else {
            add_option( "forms_agile_webstats", $webstats, $deprecated, $autoload );
        }
             $pages = get_pages(); 
             foreach ( $pages as $page ) {
                 if($webstats == "0")
                     update_post_meta($page->ID , 'forms_enable_webstats', 2);
                 else
                    update_post_meta($page->ID , 'forms_enable_webstats', 1);
             }
    }
agilecrm_forms_header($domain);
?> <div id="features">
<?php $webrules = get_option( "forms_agile_webrules" );
$webstats = get_option( "forms_agile_webstats" );
?> <form action="#" method="post" >
<?php wp_nonce_field('agilecrm_forms_nonce_action','agilecrm_forms_nonce_field'); ?>
<input type="hidden" name="featuresform" id="featuresform" value="featuresform" />
<a href="admin.php?page=agilecrm_forms_webrules" id="boxm" >
<div class="box">
  <div class="right stripline">
   ...
   <a href="admin.php?page=agilecrm_forms_webrules" class="more">More</a>
  </div>
</div></a>
<a href="admin.php?page=agilecrm_forms_formbuilder" id="boxm">
<div class="box">
  <div class="right stripline">
   ...
   <a href="admin.php?page=agilecrm_forms_formbuilder" class="more">More</a>
   </div> 
 </div></a>
<a href="admin.php?page=agilecrm_forms_landing" id="boxm"> 
   <div class="box">
   <div class="right stripline">
   ...
   <a href="admin.php?page=agilecrm_forms_landing" class="more">More</a>
 </div>
</div> </a>
<a href="admin.php?page=agilecrm_forms_email" id="boxm">
  <div class="box">
   <div class="right stripline">
    ...
    <a href="admin.php?page=agilecrm_forms_email" class="more">More</a>
   </div>
</div> </a>
<a href="admin.php?page=agilecrm_forms_webstats" id="boxm">
<div class="box">
  <div class="right stripline">
    ...
    <a href="admin.php?page=agilecrm_forms_webstats" class="more">More</a>
  </div> 
</div></a> 
<a href="admin.php?page=agilecrm_forms_refer" id="boxm">
  <div class="box">
    <div class="right stripline">
    ...
    <a class="more"  href="admin.php?page=agilecrm_forms_refer">Refer</a>
   </div>
</div></a>
</form>
</div>
...
<?php 
agilecrm_forms_customfeautre_js();
agilecrm_forms_customsubmit_js();
 }
}
function agilecrm_forms_webstats_page(){
     $rest_api = (sanitize_text_field(get_option( "forms_agile_rest_api" )));
    $js_api = (sanitize_text_field(get_option( "forms_agile_js_api" )));
    $domain = (sanitize_text_field(get_option( "forms_agile_domain" )));
    $email= (sanitize_email(get_option( "forms_agile_email" )));
    ...
    if(isset($_POST["featuresform"]) && check_admin_referer( 'agilecrm_forms_nonce_action', 'agilecrm_forms_nonce_field' )){
        $webstats= intval($_POST["webstats"]);
        if ( get_option( "forms_agile_webstats" ) !== false ) {
            update_option( "forms_agile_webstats", $webstats);
        } else {
...
<div class="clear"></div>
<?php agilecrm_forms_customfeautre_js(); ?>
</div>
<?php }
}
function agilecrm_forms_webrules_page(){
    $rest_api = (sanitize_text_field(get_option( "forms_agile_rest_api" )));
    $js_api = (sanitize_text_field(get_option( "forms_agile_js_api" )));
    $domain = (sanitize_text_field(get_option( "forms_agile_domain" )));
    $email= (sanitize_email(get_option( "forms_agile_email" )));
    ...
    $forms_agile_email = (sanitize_email(get_option( "forms_agile_email" )));
    $forms_agile_password = (sanitize_text_field(get_option( "forms_agile_password" )));
    agilecrm_forms_custom_pubnub_refresh_js ();
?><div id="features">
<div class="mainLeftbox">
...
function agilecrm_forms_landing_page(){
    $rest_api = (sanitize_text_field(get_option( "forms_agile_rest_api" )));
    $js_api = (sanitize_text_field(get_option( "forms_agile_js_api" )));
    $domain = (sanitize_text_field(get_option( "forms_agile_domain" )));
    $email= (sanitize_email(get_option( "forms_agile_email" )));
...
function agilecrm_forms_formbuilder_page(){
    $rest_api = (sanitize_text_field(get_option( "forms_agile_rest_api" )));
    $js_api = (sanitize_text_field(get_option( "forms_agile_js_api" )));
    $domain = (sanitize_text_field(get_option( "forms_agile_domain" )));
    $email= (sanitize_email(get_option( "forms_agile_email" )));
...
function agilecrm_forms_refer_page(){
    $domain = (sanitize_text_field(get_option( "forms_agile_domain" )));
    agilecrm_forms_header($domain); ?>
<div id="features">
<div class="mainLeftbox">
...
<?php }
function agilecrm_forms_email_page(){
   $rest_api = (sanitize_text_field(get_option( "forms_agile_rest_api" )));
    $js_api = (sanitize_text_field(get_option( "forms_agile_js_api" )));
    $domain = (sanitize_text_field(get_option( "forms_agile_domain" )));
    $email= (sanitize_email(get_option( "forms_agile_email" )));
...
function agilecrm_forms_settings_page(){
    $deprecated = null;
    $autoload = 'no';
   echo $getdoamin = sanitize_text_field($_GET["domain"]);
    ...
    if($getdoamin){
        ...
        $js_api = $js_api_key;
        if($rest_api){
        if ( get_option( "forms_agile_rest_api" ) !== false ) {
            update_option( "forms_agile_rest_api", $rest_api);
        } else {
            add_option( "forms_agile_rest_api", $rest_api, $deprecated, $autoload );
        }

        if ( get_option( "forms_agile_js_api" ) !== false ) {
            update_option( "forms_agile_js_api", $js_api);
        } else {
            add_option( "forms_agile_js_api", $js_api, $deprecated, $autoload );
        }
        if ( get_option( "forms_agile_domain" ) !== false ) {
            update_option( "forms_agile_domain", $domain);
        } else {
            add_option( "forms_agile_domain", $domain, $deprecated, $autoload );
        }

        if ( get_option( "forms_agile_email" ) !== false ) {
            update_option( "forms_agile_email", $email);
        } else {
            add_option( "forms_agile_email", $email, $deprecated, $autoload );
        }
        $hash_password = wp_hash_password( $password );
        if ( get_option( "forms_agile_password" ) !== false ) {
            update_option( "forms_agile_password", $hash_password);
        } else {
            add_option( "forms_agile_password", $hash_password, $deprecated, $autoload );
        } 
    ...
    if($_POST["save"] && check_admin_referer( 'agilecrm_forms_nonce_action', 'agilecrm_forms_nonce_field' )){
    ...
    <div class="well">
    <form action="?page=agilecrm_forms_settings" method="post" >
...
<?php }
add_action( 'load-post.php', 'agilecrm_forms_page_post_meta_boxes_setup' );
add_action( 'load-post-new.php', 'agilecrm_forms_page_add_post_meta_boxes' );
add_action( 'save_post', 'agilecrm_forms_page_save_postdata' );

function agilecrm_forms_page_post_meta_boxes_setup() {
    add_action( 'add_meta_boxes', 'agilecrm_forms_page_add_post_meta_boxes' );
}

function agilecrm_forms_page_add_post_meta_boxes() {
    add_meta_box('page_section',__('Agilecrm Section', 'page_metabox' ),'agilecrm_forms_page_post_box','page','advanced','high');
}

function agilecrm_forms_page_save_postdata( $post_id ) {
 
    if(isset($_POST["forms_enable_webrule"])){
        $forms_enable_webrule  = intval($_POST["forms_enable_webrule"]);
        update_post_meta($post_id , 'forms_enable_webrule', $forms_enable_webrule);
    }

    if(isset($_POST["forms_enable_webstats"])){
        $forms_enable_webstats = intval($_POST["forms_enable_webstats"]);
        update_post_meta($post_id , 'forms_enable_webstats', $forms_enable_webstats);
    }

    if(isset($_POST["forms_landing_page"])){
        $forms_landing_page = sanitize_text_field($_POST["forms_landing_page"]);
        update_post_meta($post_id , 'forms_landing_page', $forms_landing_page);
    }
}

function agilecrm_forms_page_post_box( $post ) {
    echo "<style>";
    echo "#page-list label{width:50%;float:left}";
    echo "</style>";
    echo "<ul id='page-list'>";
        echo "<li>";
        echo '<label for="forms_enable_webrule">'.__("Enable Web Rule :");
        echo '</label> ';
        $forms_enable_webrule = "";
        if(get_post_meta($post->ID, 'forms_enable_webrule',true) == 1 || get_post_meta($post->ID, 'forms_enable_webrule',true) == "")
            $forms_enable_webrule = " selected " ;

        echo '<select id="forms_enable_webrule" autocomplete="off" name="forms_enable_webrule">';

        echo '<option value="2">No</option>';
        echo '<option value="1" '.$forms_enable_webrule.'>Yes</option>';
        echo '</select>';

        echo "</li>";

        echo "<li>";
        echo '<label for="forms_enable_webstats">'.__("Enable Web Stats :");
        echo '</label> ';
        $forms_enable_webstats = "";

        if(get_post_meta($post->ID, 'forms_enable_webstats',true) == 1 || get_post_meta($post->ID, 'forms_enable_webstats',true) == "")
            $forms_enable_webstats= " selected " ;

 
        echo '<select id="forms_enable_webstats" autocomplete="off" name="forms_enable_webstats">';

        echo '<option value="2">No</option>';
        echo '<option value="1" '.$forms_enable_webstats.'>Yes</option>';
        echo '</select>';
        echo "</li>";
        ...
        if(count($result) > 0){   
        echo "<li>";
        echo '<label for="forms_landing_page">'.__("Landing Page :");
        echo '</label> ';
        $forms_landing_page = get_post_meta($post->ID, 'forms_landing_page',true);
        echo '<select id="forms_landing_page" autocomplete="off" name="forms_landing_page">';
        echo '<option value="">Select</option>';
        foreach($result as $k => $v){
            if($forms_landing_page == $v->id)
        ...
        echo '<label for="forms_landing_page">'.__("Landing Page :");
        echo '</label> ';
        echo '<select id="forms_landing_page" autocomplete="off" name="forms_landing_page" disabled>';
...
add_action('admin_head', 'agilecrm_forms_button');
function agilecrm_forms_button() {
    global $typenow;
    ...
    if ( get_user_option('rich_editing') == 'true') {
        add_filter("mce_external_plugins", "agilecrm_forms_add_tinymce_plugin");
        add_filter('mce_buttons', 'agilecrm_forms_register_button');
    }
}

function agilecrm_forms_add_tinymce_plugin($plugin_array) {
    $plugin_array['agilecrm_forms_button'] = plugins_url( '/js/agile.js', __FILE__ ); // CHANGE THE BUTTON SCRIPT HERE
    return $plugin_array;
}

function agilecrm_forms_register_button($buttons) {
   array_push($buttons, "agilecrm_forms_button");
   return $buttons;
}

add_shortcode('agileform_forms','agileform_forms');
function agileform_forms($atts,$content,$tag){
    if(isset($atts["id"])){
...
function agilecrm_forms_custom_pubnub_refresh_js() {
   $rest_api = (sanitize_text_field(get_option( "forms_agile_rest_api" )));
     $page = $_GET['page'];
   if($page =='agilecrm_forms_webrules' || $page =='agilecrm_forms_landing' || $page =='agilecrm_forms_formbuilder' || $page =='agilecrm_forms_email'){
        ...
        wp_add_inline_script( 'pubnub_autorefresh', '
          ...
          loadScript("'.plugins_url('/js/pubnub-3.4.min.js', __FILE__).'", function() {
              ...
              Agile_Pubnub.subscribe({ channel : getAgileChannelName(), restore : false, message : function(message, env, channel)
              {
                  ...
                  if(name== "WebRule"){
                    window.location.href = "admin.php?page=agilecrm_forms_webrules";
                  }else if(name== "LandingPages"){
                    window.location.href = "admin.php?page=agilecrm_forms_landing";
                  }else if(name=="Forms"){
                    window.location.href = "admin.php?page=agilecrm_forms_formbuilder";
                  }else if(name=="Campaigns"){
                    window.location.href = "admin.php?page=agilecrm_forms_email";
                  }
              }});  
          });
...
add_action ( 'admin_enqueue_scripts', 'agilecrm_forms_custom_pubnub_refresh_js');


function agilecrm_forms_footer() {
    global $post;
    $js_api = (sanitize_text_field(get_option( "forms_agile_js_api" )));
    $domain= (sanitize_text_field(get_option( "forms_agile_domain" )));
    $rest_api = (sanitize_text_field(get_option( "forms_agile_rest_api" )));
    ...
    $webrules = get_option( "forms_agile_webrules" );
    $webstats = get_option( "forms_agile_webstats" );
    $pagewebrule = get_post_meta($post->ID, 'forms_enable_webrule',true);
    $pagewebstats=get_post_meta($post->ID, 'forms_enable_webstats',true);
    $agile_url = "https://" .$domain. ".agilecrm.com/dev/api/";
    $headers = array(
                'Authorization' => 'Basic ' . base64_encode( $email. ':' .$rest_api ),
                'Content-type' => 'application/json',
...
add_action( 'wp_footer', 'agilecrm_forms_footer');
function agilecrm_forms_customfeautre_js() {
  $page = $_GET['page'];
    if($page != 'agilecrm_forms_settings'){
        wp_enqueue_script('feautre',plugins_url('/js/inputsubmit.js', __FILE__),array('jquery') );
      }
    }
   add_action( 'wp_enqueue_scripts',  'agilecrm_forms_customfeautre_js');
function agilecrm_forms_customsubmit_js() {
        wp_enqueue_script('formsubmit',plugins_url('/js/submitclassform.js', __FILE__),array('jquery') );
    }
   add_action( 'admin_enqueue_scripts',  'agilecrm_forms_customsubmit_js');

function agilecrm_forms_custom_js() {
 ...
 add_action( 'admin_enqueue_scripts',  'agilecrm_forms_custom_js');

function agilecrm_forms_hidedata_js() {
        wp_enqueue_script('hidedata',plugins_url('/js/hidedata.js', __FILE__),array('jquery') );
    }
add_action( 'admin_enqueue_scripts',  'agilecrm_forms_hidedata_js');
/*function agilecrm_forms_customrefresh_js() {
  $page = $_GET['page'];
  $get_domain = $_GET['domain'];
    if($page == 'agilecrm_forms_settings' && $_POST["save"])
    {
          wp_enqueue_script('refresh',plugins_url('/js/refreshpage.js', __FILE__),array('jquery') );
    }
  }
add_action( 'admin_enqueue_scripts',  'agilecrm_forms_customrefresh_js');

function agilecrm_forms_get_customrefresh_js() {
  $page = $_GET['page'];
  $get_domain = $_GET['domain'];
    if($page == 'agilecrm_forms_settings' && $get_domain && $_POST["save"])
    {
          wp_enqueue_script('refresh',plugins_url('/js/refreshpage.js', __FILE__),array('jquery') );
    }
 }
add_action( 'admin_enqueue_scripts',  'agilecrm_forms_get_customrefresh_js');*/
?>